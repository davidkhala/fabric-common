...
export default class configtxlator extends BinManager {
	/**
	 ...
	 */
	async computeUpdateFile(channelID, original, updated, outputFile) {
	 ...
	 */
	async computeUpdate(channelID, original_config_proto, updated_config_proto) {
		...
		await this.computeUpdateFile(channelID, tmpFileOriginal, tmpFileUpdated, tmpFileOutput);
	 ...
	 */
	async server(action, {hostname = '0.0.0.0', port = 7059, CORS = ['*']} = {}) {
		if (action === 'start') {
